(()=>{
  ...
  function markVersion(){document.querySelectorAll('body *').forEach(el=>{if(el.children.length===0&&/Wersja aplikacji:\s*v\d+\.\d+\.\d+/.test(el.textContent||''))el.textContent=(el.textContent||'').replace(/Wersja aplikacji:\s*v\d+\.\d+\.\d+/,'Wersja aplikacji: v1.30.18')})}
