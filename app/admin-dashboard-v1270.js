(()=>{
  ...
  function fits(pos,layout,ignoreKey=null){
    const cols=columns();
    if(pos.c<1||pos.r<1||pos.w<1||pos.h<1||pos.c+pos.w-1>cols)return false;
    return !Object.entries(layout).some(([k,p])=>k!==ignoreKey&&p&&overlaps(pos,p));
  }
  function firstFree(span,layout,startRow=1,startCol=1){
    const cols=columns();
    for(let r=Math.max(1,startRow);r<200;r++){
      for(let c=(r===startRow?Math.max(1,startCol):1);c<=cols-span.w+1;c++){
        const p={c,r,w:span.w,h:span.h};if(fits(p,layout))return p;
      }
    }
    return{c:1,r:200,w:span.w,h:span.h};
  }

  function normalizedLayout(){
    const saved=readLayout(),out={};
    const list=cards();
    list.forEach(el=>{
  ...
  function rowHeight(){
    if(!grid)return 168;
    const v=parseFloat(getComputedStyle(grid).gridAutoRows);return Number.isFinite(v)&&v>0?v:168;
  }
  ...
  function showPlaceholder(cell){
    if(!grid||!cell)return;
    if(!placeholder){placeholder=document.createElement('div');placeholder.className='cf-admin-grid-placeholder';grid.appendChild(placeholder)}
    placeholder.style.gridColumn=`${cell.c} / span ${cell.w}`;
    placeholder.style.gridRow=`${cell.r} / span ${cell.h}`;
    lastCell=cell;
  }
  ...
  function bindGrid(g){
    grid=g;applyLayout();bindDesktop();
    if(g.dataset.cfGridObserver!=='1'){
      g.dataset.cfGridObserver='1';
      new MutationObserver(m=>{if(active)return;if(m.some(x=>[...x.addedNodes,...x.removedNodes].some(n=>n?.classList?.contains?.('cf-company-card'))))setTimeout(applyLayout,0)}).observe(g,{childList:true});
    }
  }
  ...
  function boot(){
    ...
    document.addEventListener('visibilitychange',()=>{if(!document.hidden){refreshAux();setTimeout(applyLayout,80)}},{passive:true});
  }
