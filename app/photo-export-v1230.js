(()=>{
  ...
  async function tx(mode,fn){
    ...
    return new Promise((resolve,reject)=>{
      ...
      t.oncomplete=()=>resolve();t.onerror=()=>reject(t.error||new Error('Błąd lokalnej bazy'));t.onabort=()=>reject(t.error||new Error('Przerwano zapis lokalny'));
    });
  }
  ...
  async function persistStaged(recordId,kind){
    const k=keyFor(recordId,kind),files=[...(staged.get(k)||[])];if(!files.length)return 0;
    const ts=Date.now();
    ...
    staged.delete(k);return files.length;
  }

  async function metadata(recordId){
    if(typeof cfSupabase==='undefined')throw new Error('Brak dostępu do danych wpisu.');
    const {data,error}=await cfSupabase.from('wash_records').select('plate,type,order_date,order_due_date,wash_date,schedule_proposed_date').eq('id',recordId).single();if(error)throw error;
  ...
  async function makeCombinedZip(recordId){
    const items=await byRecord(recordId),before=items.filter(x=>x.kind==='przed'),after=items.filter(x=>x.kind==='po');
    ...
    for(let i=0;i<ordered.length;i++){
      const {x,path}=ordered[i],bytes=new Uint8Array(x.bytes),nameBytes=enc.encode(path),crc=crc32(bytes),size=bytes.byteLength;
    ...
    const meta=await metadata(recordId),blob=new Blob([...locals,...centrals,end],{type:'application/zip'});
  ...
  async function renderLocal(){
    ...
    box.querySelector('[data-local-export]').style.display=before.length&&after.length?'':'none';const grid=box.querySelector('[data-local-grid]');grid.innerHTML='';const urls=[];
    for(const p of visible){const blob=new Blob([p.bytes],{type:p.mime||'image/jpeg'}),url=URL.createObjectURL(blob);urls.push(url);const cell=document.createElement('div');cell.className='cf-photo-local-thumb';cell.innerHTML='<img alt=""><button type="button" aria-label="Usuń">×</button>';cell.querySelector('img').src=url;cell.querySelector('button').onclick=async()=>{await del(p.id);renderLocal()};grid.appendChild(cell)}
    setTimeout(()=>urls.forEach(u=>URL.revokeObjectURL(u)),30000);
  }

  async function saveCurrent(){
    if(!currentRecordId||busy)return;const kind=activeKind();busy=true;const btn=document.querySelector('#cfPhotoOverlay [data-photo-save]');if(btn){btn.disabled=true;btn.textContent='Zapisywanie lokalnie…'}
    try{const n=await persistStaged(currentRecordId,kind);if(!n){toast('Brak nowych zdjęć do zapisania.');return}toast(`Zapisano lokalnie ${n} ${n===1?'zdjęcie':'zdjęć'} ${kind.toUpperCase()}.`);await renderLocal();const before=await byRecordKind(currentRecordId,'przed'),after=await byRecordKind(currentRecordId,'po');if(kind==='po'&&before.length&&after.length)await exportCombined(currentRecordId)}catch(e){console.error(e);toast(e?.message||'Nie udało się zapisać zdjęć lokalnie.')}finally{if(btn){btn.disabled=false;btn.textContent='Zapisz zdjęcia'}busy=false}
  }

  async function listAllStoragePaths(prefix=''){
    const out=[];let offset=0;while(true){const {data,error}=await cfSupabase.storage.from(BUCKET).list(prefix,{limit:100,offset,sortBy:{column:'name',order:'asc'}});if(error)throw error;const rows=data||[];if(!rows.length)break;for(const item of rows){const path=prefix?`${prefix}/${item.name}`:item.name;if(item.id)out.push(path);else out.push(...await listAllStoragePaths(path))}if(rows.length<100)break;offset+=rows.length}return out;
  }
  async function cleanupServerPhotosOnce(){
    if(localStorage.getItem(CLEANUP_KEY)==='1')return;for(let i=0;i<80;i++){if(typeof cfSupabase!=='undefined'&&cfSupabase?.storage)break;await new Promise(r=>setTimeout(r,250))}if(typeof cfSupabase==='undefined'||!cfSupabase?.storage)return;
    try{const paths=await listAllStoragePaths('');for(let i=0;i<paths.length;i+=50){const batch=paths.slice(i,i+50),{error}=await cfSupabase.storage.from(BUCKET).remove(batch);if(error)throw error}localStorage.setItem(CLEANUP_KEY,'1');console.info(`CleanFleet: usunięto ${paths.length} starych plików zdjęć z serwera.`)}catch(e){console.warn('CleanFleet cleanup zdjęć z serwera',e)}
  }

  function events(){
    document.addEventListener('click',e=>{const op=e.target.closest?.('[data-cf-photos]');if(op){currentRecordId=op.dataset.cfPhotos||null;setTimeout(()=>renderLocal().catch(()=>{}),180);return}const rm=e.target.closest?.('[data-photo-remove]');if(rm&&currentRecordId){removeStaged(currentRecordId,activeKind(),Number(rm.dataset.photoRemove));return}const tab=e.target.closest?.('#cfPhotoOverlay .cf-photo-tab');if(tab){setTimeout(()=>renderLocal().catch(()=>{}),80);return}const save=e.target.closest?.('[data-photo-save]');if(save&&currentRecordId){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();saveCurrent()}},true);
  ...
  async function start(){ensureStyles();events();try{await navigator.storage?.persist?.()}catch(_){ }cleanupServerPhotosOnce()}
