(()=>{
  ...
  async function getPhotoRows(recordId,kind){
    ...
    const {data,error}=await sb.from('wash_record_photos').select('id,wash_record_id,kind,storage_path,content_hash,created_at,created_by').eq('wash_record_id',recordId).eq('kind',kind).order('created_at');
    if(error)throw error;
    return data||[];
  }
  async function backfillLegacyHashes(rows){
    const sb=await waitForSupabase();
  ...
  async function photoBasePath(recordId,rows){
    ...
    const sb=await waitForSupabase();
    const {data,error}=await sb.from('wash_records').select('id,plate,type,order_date,order_due_date,wash_date,schedule_proposed_date').eq('id',recordId).single();
    if(error)throw error;
  ...
  function isAlreadyExistsError(e){
    const s=[e?.message,e?.error,e?.statusCode,e?.status,e?.code].filter(Boolean).join(' ').toLowerCase();
  ...
  async function objectExists(path){
    ...
    try{
      const {data,error}=await sb.storage.from(PHOTO_BUCKET).list(folder,{limit:20,search:name});
      if(!error&&Array.isArray(data))return data.some(x=>x.name===name);
    }catch(_){ }
    try{
  ...
  async function deleteMeta(id){
    if(!id)return;
    const sb=await waitForSupabase();
    try{await sb.from('wash_record_photos').delete().eq('id',id)}catch(_){ }
  }
  ...
  async function processOne(item){
    ...
    rows=await backfillLegacyHashes(rows);

    const existing=rows.find(r=>r.content_hash===hash);
    if(existing){
      if(await objectExists(existing.storage_path)){
        item.status='skipped';item.error='';item.updatedAt=now();item.finishedAt=now();await put(item);updateUiSoon();return;
      }
      await deleteMeta(existing.id);
    ...
    let meta=null;
    let ins=await sb.from('wash_record_photos').insert({wash_record_id:item.recordId,kind:item.kind,storage_path:path,content_hash:hash}).select('id,storage_path').maybeSingle();
    if(ins.error){
      if(!isAlreadyExistsError(ins.error))throw ins.error;
      const {data:dup,error:de}=await sb.from('wash_record_photos').select('id,storage_path').eq('wash_record_id',item.recordId).eq('kind',item.kind).eq('content_hash',hash).maybeSingle();
      if(de)throw de;
      if(dup&&await objectExists(dup.storage_path)){
        item.status='skipped';item.error='';item.updatedAt=now();item.finishedAt=now();await put(item);updateUiSoon();return;
      }
      if(dup)await deleteMeta(dup.id);
      ins=await sb.from('wash_record_photos').insert({wash_record_id:item.recordId,kind:item.kind,storage_path:path,content_hash:hash}).select('id,storage_path').single();
    ...
    if(up.error&&!isAlreadyExistsError(up.error)){
      await deleteMeta(meta?.id);
      throw up.error;
    }

    item.status='done';item.error='';item.updatedAt=now();item.finishedAt=now();item.attempts=item.attempts||0;await put(item);updateUiSoon();
  }
  ...
  async function processor(){
    ...
    try{
      while(!manualPaused()&&navigator.onLine){
        const item=await nextRunnable();if(!item)break;
  ...
  function stageFiles(recordId,kind,files){const k=keyFor(recordId,kind),arr=staged.get(k)||[];arr.push(...files);staged.set(k,arr)}
  function removeStaged(recordId,kind,index){const k=keyFor(recordId,kind),arr=staged.get(k)||[];if(index>=0&&index<arr.length)arr.splice(index,1);staged.set(k,arr)}
  function takeStaged(recordId,kind){const k=keyFor(recordId,kind),arr=[...(staged.get(k)||[])];staged.delete(k);return arr}

  async function latestBatchStats(recordId){
    const items=await itemsByRecord(recordId);if(!items.length)return null;
    ...
    const retryCount=batch.reduce((n,x)=>n+(x.attempts||0),0);
    const lastError=[...batch].sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0)).find(x=>x.error)?.error||'';
    return{total:batch.length,done,skipped,queued,retryCount,lastError,finished:done+skipped===batch.length};
  }

  function ensureStyles(){
    if(document.getElementById('cfPhotoQueue1211Styles'))return;
    const s=document.createElement('style');s.id='cfPhotoQueue1211Styles';s.textContent=`
      ...
      .cf-photo-durable-error{font-size:9px;color:#b7483c;margin-top:5px;word-break:break-word}
      ...
      .cf-photo-durable-actions{display:flex;gap:7px;margin-top:8px}.cf-photo-durable-actions button{border:1px solid #ccd1c7;background:#fff;border-radius:8px;padding:7px 10px;font-size:10px;font-weight:900;cursor:pointer}.cf-photo-durable-actions .primary{background:#16150f;color:#fff;border-color:#16150f}
    `;document.head.appendChild(s);
  }
  async function renderQueueUi(){
    const modal=document.getElementById('cfPhotoOverlay');if(!modal||!modal.classList.contains('open')||!currentRecordId)return;
    const status=modal.querySelector('[data-photo-status]');if(!status)return;
    let box=modal.querySelector('.cf-photo-durable');const stats=await latestBatchStats(currentRecordId);
    if(!stats){box?.remove();return}
    if(!box){
      box=document.createElement('div');box.className='cf-photo-durable';
      box.innerHTML='<div class="cf-photo-durable-head"><span data-q-label>Kolejka zdjęć</span><span data-q-count>0 z 0</span></div><div class="cf-photo-durable-meta" data-q-meta></div><div class="cf-photo-durable-error" data-q-error></div><div class="cf-photo-durable-track"><div class="cf-photo-durable-bar" data-q-bar></div></div><div class="cf-photo-durable-actions"><button type="button" data-q-pause>Pauza</button><button type="button" class="primary" data-q-resume>Wznów</button></div>';
    ...
    const completed=stats.done+stats.skipped,pct=stats.total?Math.round(completed/stats.total*100):0,paused=manualPaused()||!navigator.onLine;
    ...
    const parts=[];if(stats.skipped)parts.push(`pominięte duplikaty: ${stats.skipped}`);if(stats.queued)parts.push(`pozostało: ${stats.queued}`);if(stats.retryCount)parts.push(`ponowne próby: ${stats.retryCount}`);if(!navigator.onLine)parts.push('brak internetu');
    box.querySelector('[data-q-meta]').textContent=parts.join(' · ')||'Wszystkie zdjęcia zapisane.';
    box.querySelector('[data-q-error]').textContent=stats.lastError&&!stats.finished?`Ostatni błąd: ${stats.lastError}`:'';
  ...
  function reopenPhotos(recordId){
    document.querySelector('#cfPhotoOverlay [data-photo-close]')?.click();
    setTimeout(()=>{
      const b=[...document.querySelectorAll('[data-cf-photos]')].find(x=>x.dataset.cfPhotos===recordId);b?.click();setTimeout(updateUiSoon,180);
    },90);
  }
  ...
  function captureEvents(){
    document.addEventListener('click',e=>{
      ...
      if(save&&currentRecordId){
        ...
        (async()=>{
          ...
          try{await enqueueBatch(currentRecordId,kind,files);toast(`Dodano ${files.length} ${files.length===1?'zdjęcie':'zdjęć'} do trwałej kolejki.`);const id=currentRecordId;reopenPhotos(id);kick()}
    ...
    document.addEventListener('change',e=>{
      const input=e.target.closest?.('[data-photo-input]');if(!input||!currentRecordId)return;
      const files=[...(input.files||[])];if(files.length)stageFiles(currentRecordId,activeKind(),files);
    },true);
