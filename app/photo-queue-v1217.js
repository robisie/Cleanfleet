(()=>{
  ...
  async function setPhase(item,phase){item.phase=phase;item.updatedAt=now();await put(item);updateUiSoon()}
  ...
  async function durableBlob(item){
    ...
    if(item.blob){
      try{
        const ab=await withTimeout(item.blob.arrayBuffer(),12000,'Odczyt lokalnej kopii');
        ...
        item.status='unavailable';item.error='Lokalna kopia zdjęcia jest niedostępna po ponownym uruchomieniu iOS. Wybierz zdjęcie ponownie.';item.phase='niedostępne lokalnie';item.updatedAt=now();item.finishedAt=now();await put(item);updateUiSoon();return null;
      }
    }
    item.status='unavailable';item.error='Brak lokalnej kopii zdjęcia. Wybierz zdjęcie ponownie.';item.phase='niedostępne lokalnie';item.updatedAt=now();item.finishedAt=now();await put(item);updateUiSoon();return null;
  }
  ...
  async function processOne(item){
    item.status='uploading';item.error='';item.updatedAt=now();item.phase='przygotowanie';await put(item);updateUiSoon();
    ...
    const hash=await hashBlob(blob);item.contentHash=hash;item.updatedAt=now();await put(item);
    await setPhase(item,'sprawdzanie duplikatu');
    const sb=await waitForSupabase();let rows=await getRows(item.recordId,item.kind);
    const same=rows.find(r=>r.content_hash===hash);
    if(same){
      try{const d=await withTimeout(sb.storage.from(PHOTO_BUCKET).download(same.storage_path),25000,'Sprawdzanie duplikatu w Storage');if(!d.error&&d.data){item.status='skipped';item.phase='duplikat pominięty';item.finishedAt=now();item.updatedAt=now();await put(item);updateUiSoon();return}}catch(_){ }
      try{await withTimeout(sb.from('wash_record_photos').delete().eq('id',same.id),20000,'Usuwanie niepełnego rekordu')}catch(_){ }
    ...
    const up=await withTimeout(sb.storage.from(PHOTO_BUCKET).upload(path,blob,{contentType:'image/jpeg',upsert:false,cacheControl:'3600'}),45000,'Wysyłanie zdjęcia');
    ...
    item.status=ins.error?'skipped':'done';item.phase=ins.error?'duplikat pominięty':'zapisane';item.error='';item.finishedAt=now();item.updatedAt=now();await put(item);updateUiSoon();
  }
  ...
  async function processor(){
    if(processing||paused()||!navigator.onLine)return;processing=true;
    try{
      while(!paused()&&navigator.onLine){
        ...
        try{await processOne(item)}catch(e){item.status='queued';item.phase='oczekuje na ponowną próbę';item.attempts=(item.attempts||0)+1;item.error=e?.message||String(e);item.updatedAt=now();item.nextAttemptAt=now()+retryDelay(item.attempts);await put(item);updateUiSoon();if(!navigator.onLine)break}
      }
  ...
  async function recover(){
    ...
    for(const x of a){
      if(x.status==='uploading'){x.status='queued';x.phase='wznowienie po przerwaniu';x.nextAttemptAt=0;x.updatedAt=now();await put(x)}
      if(['done','skipped'].includes(x.status)&&x.finishedAt&&now()-x.finishedAt>7*86400000)await del(x.id);
      if(x.status==='queued'&&x.blob&&!x.bytes)await durableBlob(x);
    }
  }
  ...
  async function enqueue(recordId,kind){
    ...
    const batchId=uid(),ts=now();for(let i=0;i<ok.length;i++){const f=ok[i];await put({id:uid(),batchId,recordId,kind,bytes:f.bytes,mime:f.mime,originalName:f.name,status:'queued',phase:'oczekuje',attempts:0,error:'',createdAt:ts+i,updatedAt:ts,nextAttemptAt:0,finishedAt:0,storagePath:'',contentHash:''})}return ok.length
  }
  ...
  async function renderUi(){
    ...
    box.querySelector('[data-q-count]').textContent=`${completed} z ${x.total}`;box.querySelector('[data-q-bar]').style.width=`${pct}%`;box.querySelector('[data-q-label]').textContent=x.finished?'✓ Kolejka zakończona':paused()||!navigator.onLine?'Kolejka wstrzymana':'Wysyłanie zdjęć…';
    ...
    box.querySelector('[data-q-error]').textContent=x.err&&!x.finished?`Ostatni błąd: ${x.err}`:(x.unavailable?'Część starych zdjęć utraciła dostęp po restarcie iOS — wybierz je ponownie.':'');
