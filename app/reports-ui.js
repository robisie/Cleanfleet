...
let cfg={source:'wash_records',title:'Raport CleanFleet',dateField:'wash_date',from:'',to:'',filters:[],groups:[],columns:['company_name','plate','type','wash_date','performed_by','cost','approved','paid','invoice_invoice_number'],metricIds:['count','completed','vehicles','sum','avg'],chart:'bar',chartMetric:'count',resultMode:'full'};
...
const COLUMN_GROUPS=[
 {id:'basic',label:'Dane podstawowe',hint:'Pojazd, firma, osoby i kwota',open:true},
 {id:'status',label:'Status i rozliczenie',hint:'Wykonanie, płatność, priorytet i status terminu',open:true},
 {id:'dates',label:'Daty i godziny',hint:'Zlecenie, termin, pranie i potwierdzenia',open:true},
 {id:'notes',label:'Opisy i uwagi',hint:'Opisy zlecenia, wykonania i uwagi',open:true},
 {id:'vehicle',label:'Kartoteka pojazdu',hint:'Dane pobierane z kartoteki pojazdu',open:true},
 {id:'invoice',label:'Faktura',hint:'Numer, data, status i rozliczenie faktury',open:true},
 {id:'advanced',label:'Zaawansowane',hint:'Identyfikatory i pola techniczne',open:false},
 {id:'other',label:'Pozostałe',hint:'Pozostałe pola tego źródła danych',open:false}
];
function columnGroup(key,def){
 const label=String(def?.label||'').toLocaleLowerCase('pl');
 if(key.startsWith('vehicle_'))return'vehicle';
 if(key.startsWith('invoice_')||key==='has_invoice')return'invoice';
 if(['notes','completion_notes','schedule_note','note','description','vehicle_notes'].includes(key)||/opis|uwag|notatk/.test(label))return'notes';
...
function pickerLabel(group,label){
 ...
 if(group==='vehicle')text=text.replace(/^Kartoteka:\s*/i,'');
 if(group==='invoice')text=text.replace(/^Faktura:\s*/i,'');
 return text;
}
function renderColumnPicker(fields){
 const ordered=orderedFields(fields).map(([key,def],order)=>({key,def,order,group:columnGroup(key,def)}));
 const groups=COLUMN_GROUPS.map(group=>({...group,items:ordered.filter(item=>item.group===group.id)})).filter(group=>group.items.length);
 const sections=groups.map(group=>{
  const selected=group.items.filter(item=>cfg.columns.includes(item.key)).length;
  return '<details class="r-column-group" data-column-group="'+group.id+'" '+(group.open?'open':'')+'><summary><span><strong>'+esc(group.label)+'</strong><small>'+esc(group.hint)+'</small></span><span class="r-column-group-count"><b data-group-selected>'+selected+'</b>/'+group.items.length+'</span></summary><div class="r-column-group-grid">'+group.items.map(item=>'<label data-column-label data-search="'+esc((pickerLabel(group.id,item.def.label)+' '+item.key).toLocaleLowerCase('pl'))+'"><input type="checkbox" value="'+item.key+'" data-column-order="'+item.order+'" '+(cfg.columns.includes(item.key)?'checked':'')+'><span>'+esc(pickerLabel(group.id,item.def.label))+'</span></label>').join('')+'</div></details>';
 }).join('');
 return '<div class="r-column-tools"><input type="search" id="rColumnSearch" placeholder="Szukaj pola…" aria-label="Szukaj pola raportu"><div class="r-column-presets">'+(cfg.source==='wash_records'?'<button type="button" data-action="columns-preset" data-preset="basic">Podstawowy</button><button type="button" data-action="columns-preset" data-preset="billing">Rozliczenie</button><button type="button" data-action="columns-preset" data-preset="invoice">Faktura</button><button type="button" data-action="columns-preset" data-preset="vehicle">Kartoteka</button>':'')+'</div></div><div class="r-columns" id="rColumns">'+sections+'</div>';
}
function columnPreset(name,defs){
 const sets={
  basic:['company_name','plate','type','wash_date','performed_by','cost','completion_notes'],
  billing:['company_name','plate','type','wash_date','performed_by','cost','approved','paid','invoice_invoice_number','invoice_status'],
  invoice:['company_name','plate','wash_date','cost','invoice_invoice_number','invoice_invoice_date','invoice_status','invoice_paid_at'],
  vehicle:['company_name','plate','vehicle_type','vehicle_brand','vehicle_model','vehicle_production_year','vehicle_fleet_number','vehicle_driver_name']
 };
...
function vehicleSummaryRows(r){
 ...
 for(const row of (r.rows||[])){
  ...
  item.wash_count++;
  if(!item.type||item.type==='—')item.type=row.vehicle_type||row.type||'—';
  if(!item.last_wash||String(row.wash_date)>String(item.last_wash))item.last_wash=row.wash_date;
 }
 return [...map.values()].sort((a,b)=>String(a.plate).localeCompare(String(b.plate),'pl',{numeric:true})||String(a.company_name).localeCompare(String(b.company_name),'pl'));
}
const VEHICLE_SUMMARY_FIELDS={
 plate:{label:'Rejestracja'},
 type:{label:'Typ'},
 company_name:{label:'Firma / klient'},
 wash_count:{label:'Liczba prań'},
 last_wash:{label:'Ostatnie pranie'}
};
function renderVehicleSummaryResult(){
 const rows=vehicleSummaryRows(result),total=rows.reduce((sum,row)=>sum+row.wash_count,0);
 $('#rResults').innerHTML='<section class="r-box r-vehicle-summary"><div class="r-barline"><div><h2>'+esc(result.cfg.title)+'</h2><p class="r-muted">'+esc(C[result.cfg.source].label)+' · podsumowanie pojazdów · stan: '+esc(new Date(result.loadedAt).toLocaleString('pl-PL'))+'</p></div><div class="r-flex"><button data-action="pdf">PDF</button><button data-action="xlsx">XLSX</button></div></div><p class="r-muted">'+esc(describe(result).join(' · '))+'</p><div class="r-cards r-vehicle-summary-cards"><div class="r-card"><span>Pojazdy</span><strong>'+rows.length+'</strong></div><div class="r-card"><span>Wykonane prania</span><strong>'+total+'</strong></div></div><div class="r-scroll"><table><thead><tr>'+Object.keys(VEHICLE_SUMMARY_FIELDS).map(k=>'<th>'+esc(VEHICLE_SUMMARY_FIELDS[k].label)+'</th>').join('')+'</tr></thead><tbody>'+rows.map(row=>'<tr>'+Object.keys(VEHICLE_SUMMARY_FIELDS).map(k=>'<td>'+esc(display(row[k]))+'</td>').join('')+'</tr>').join('')+'</tbody></table></div><p class="r-muted r-summary-note">Liczba prań obejmuje tylko wpisy z uzupełnioną datą prania i respektuje aktualny okres oraz wszystkie filtry raportu.</p></section>';
}
function selectedMetrics(r=result){return r.metrics.filter(m=>r.cfg.metricIds.includes(m.id));}
function renderResult(){if(!result)return;if((result.cfg.resultMode||'full')==='vehicle_summary'&&result.cfg.source==='wash_records'){renderVehicleSummaryResult();return;}const ms=selectedMetrics();$('#rResults').innerHTML='<section class="r-box"><div class="r-barline"><div><h2>'+esc(result.cfg.title)+'</h2><p class="r-muted">'+esc(C[result.cfg.source].label)+' · stan: '+esc(new Date(result.loadedAt).toLocaleString('pl-PL'))+'</p></div><div class="r-flex"><button data-action="pdf">PDF</button><button data-action="xlsx">XLSX</button></div></div><p class="r-muted">'+esc(describe(result).join(' · '))+'</p><div class="r-cards">'+ms.map(m=>'<button class="r-card" data-metric="'+esc(m.id)+'"><span>'+esc(m.label)+'</span><strong>'+metricText(m)+'</strong><small>Sprawdź dane →</small></button>').join('')+'</div><p class="r-muted">Średnie pomijają brakujące kwoty. Unikalny pojazd = firma + rejestracja. Częstotliwość to liczba wykonanych prań na unikalny pojazd w wybranym zbiorze. Odstępy obejmują tylko kolejne prania tego samego pojazdu w tym zbiorze; kilka prań jednego dnia daje odstęp 0 dni.</p><div class="r-chart" id="rChartView">'+chartSVG(result)+'</div><div class="r-scroll"><table><thead><tr><th>Grupa</th>'+ms.map(m=>'<th>'+esc(m.label)+'</th>').join('')+'</tr></thead><tbody>'+result.groups.map((g,i)=>'<tr><td><button class="link" data-group="'+i+'">'+esc(g.label)+'</button></td>'+ms.map(m=>{const gm=g.metrics.find(x=>x.id===m.id);return'<td><button class="link" data-group="'+i+'" data-gmetric="'+m.id+'">'+metricText(gm)+'</button></td>';}).join('')+'</tr>').join('')+'</tbody></table></div></section><section class="r-box" id="rRows"></section>';
 host.querySelectorAll('[data-metric]').forEach(b=>b.onclick=()=>drill(result.metrics.find(m=>m.id===b.dataset.metric),null));$$('[data-group]').forEach(b=>{const action=()=>{const g=result.groups[+b.dataset.group];const m=b.dataset.gmetric?g.metrics.find(x=>x.id===b.dataset.gmetric):{rows:g.rows,label:g.label};drill(m,g);};b.onclick=action;b.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();action();}};});renderRows();}
...
function exportVehicleSummaryXLSX(r){
 if(!root.XLSX)throw Error('Biblioteka XLSX jest niedostępna. Odśwież aplikację.');
 const X=root.XLSX,wb=X.utils.book_new(),rows=vehicleSummaryRows(r),keys=Object.keys(VEHICLE_SUMMARY_FIELDS);
 const ws=X.utils.aoa_to_sheet([keys.map(k=>VEHICLE_SUMMARY_FIELDS[k].label),...rows.map(row=>keys.map(k=>row[k]))]);
 ws['!autofilter']={ref:X.utils.encode_range({s:{r:0,c:0},e:{r:Math.max(0,rows.length),c:keys.length-1}})};
 ws['!cols']=[{wch:18},{wch:16},{wch:28},{wch:14},{wch:16}];
 X.utils.book_append_sheet(wb,ws,'Pojazdy');
 const info=X.utils.aoa_to_sheet([['Raport',r.cfg.title],['Tryb','Podsumowanie pojazdów'],...describe(r).map(v=>['Warunek',v])]);
 X.utils.book_append_sheet(wb,info,'Opis');
 X.writeFile(wb,safeName(r.cfg.title)+'.xlsx',{compression:true});
}
function exportXLSX(r){if((r.cfg.resultMode||'full')==='vehicle_summary'&&r.cfg.source==='wash_records')return exportVehicleSummaryXLSX(r);if(!root.XLSX)throw Error('Biblioteka XLSX jest niedostępna. Odśwież aplikację.');const X=root.XLSX,wb=X.utils.book_new(),ms=selectedMetrics(r);function sheet(name,aoa){const ws=X.utils.aoa_to_sheet(aoa,{cellDates:true});if(aoa.length>1)ws['!autofilter']={ref:X.utils.encode_range({s:{r:0,c:0},e:{r:aoa.length-1,c:aoa[0].length-1}})};ws['!cols']=aoa[0].map(()=>({wch:24}));for(const addr of Object.keys(ws)){if(addr[0]==='!')continue;const cell=ws[addr];if(cell.t==='d')cell.z='yyyy-mm-dd hh:mm';}X.utils.book_append_sheet(wb,ws,name);}
...
async function exportVehicleSummaryPDF(r){
 const doc=setupPDF('landscape'),logo=await lightLogoSrc(),rows=vehicleSummaryRows(r),keys=Object.keys(VEHICLE_SUMMARY_FIELDS);
 let y=adminPdfHeader(doc,r,logo);
 y=adminPdfFilterLine(doc,r,y);
 doc.setTextColor(23,27,24);doc.setFontSize(12);doc.text('PODSUMOWANIE POJAZDÓW',10,y);y+=6;
 doc.setTextColor(96,103,98);doc.setFontSize(8);doc.text('Wykonane prania: '+rows.reduce((s,x)=>s+x.wash_count,0)+' · pojazdy: '+rows.length,10,y);y+=5;
 doc.autoTable({
  head:[keys.map(k=>VEHICLE_SUMMARY_FIELDS[k].label)],
  body:rows.map(row=>keys.map(k=>display(row[k]))),
  startY:y,margin:{top:38,bottom:15,left:10,right:10},
  styles:{font:'CleanFleet',fontStyle:'normal',fontSize:8,cellPadding:2.4,overflow:'linebreak',textColor:[31,35,32],lineColor:[229,233,230],lineWidth:.12},
  headStyles:{fillColor:[241,246,235],textColor:[31,38,27],fontStyle:'normal'},
  alternateRowStyles:{fillColor:[252,252,251]},rowPageBreak:'avoid',showHead:'everyPage',
  didDrawPage:()=>{if(doc.internal.getCurrentPageInfo().pageNumber>1)adminPdfHeader(doc,r,logo);}
 });
...
function buildReportPdfStage(r){
 ...
 stage.style.cssText='position:fixed;left:-100000px;top:0;width:1360px;padding:18px;background:#ffffff;box-sizing:border-box;pointer-events:none;z-index:-1;';
 ...
 css.textContent=
   '.cf-report-pdf-stage{background:#fff!important;}'+
...
async function exportPDF(r){
 if((r.cfg.resultMode||'full')==='vehicle_summary'&&r.cfg.source==='wash_records')return exportVehicleSummaryPDF(r);
