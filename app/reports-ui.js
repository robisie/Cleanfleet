...
function readConfig(){cfg.title=$('#rTitle').value.trim()||'Raport CleanFleet';cfg.dateField=$('#rDate').value;cfg.from=$('#rFrom').value;cfg.to=$('#rTo').value;cfg.amountField=$('#rAmount').value;cfg.distinctField=$('#rDistinct').value;cfg.chart=$('#rChart').value;cfg.chartMetric=$('#rChartMetric').value;cfg.columns=$('#rColumns input:checked').sort((a,b)=>Number(a.dataset.columnOrder||0)-Number(b.dataset.columnOrder||0)).map(x=>x.value);cfg.metricIds=$$('#rMetrics input:checked').map(x=>x.value);}
