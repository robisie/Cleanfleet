...
function pdfHistoryCard(doc,r,index,y){
 const pw=doc.internal.pageSize.getWidth(),status=vehicleStatus(r),pending=!r.wash_date,notes=display(r.notes),noteLines=doc.splitTextToSize(notes,pw-48),h=Math.max(61,53+Math.min(noteLines.length,3)*4);
 ...
 const ny=y+53;doc.setFillColor(249,250,250);doc.roundedRect(12,ny-4,pw-24,h-(ny-y)+1,1.4,1.4,'F');pdfText(doc,'Opis / uwagi',16,ny,7,[94,103,113]);doc.setFontSize(7.8);doc.setTextColor(35,39,36);doc.text(noteLines.slice(0,3),16,ny+5);return h;
}
