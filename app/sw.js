// CleanFleet v1.30.64 — aktualny panel administratora także przed przejęciem strony przez SW.
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith((async () => {
    ...
    html = html.replace(/Wersja aplikacji:\s*v\d+\.\d+\.\d+(?:\s*beta)?/g, 'Wersja aplikacji: v1.30.64');
    html = html.replace(/\/app\/reports\.css\?v=[^"']+/g, '/app/reports.css?v=13064');
    html = html.replace(/\/app\/reports-ui\.js\?v=[^"']+/g, '/app/reports-ui.js?v=13064');

    if (!html.includes('/app/glass-icons.css')) {
      html = html.replace('</head>', '<link rel="stylesheet" href="/app/glass-icons.css?v=13064">\n</head>');
    }
    if (!html.includes('/app/glass-icons.js')) {
      html = html.replace('</body>', '<script src="/app/glass-icons.js?v=13064"><\/script>\n</body>');
    }
...
<!-- CF_WEATHER_SLOT_END -->`;
    ...
    const injectedScripts = '<script src="/app/weather-v6.js?v=20260916-1"></script>\n' +
      ...
      '<script src="/app/photo-camera-v1213.js?v=20260922-13064"></script>\n' +
      ...
      '<script src="/app/admin-dashboard-v1270.js?v=20260922-13064"></script>';
