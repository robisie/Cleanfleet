(() => {
  ...
  function classify(d){
    ...
    if(heavyRain) return {dot:RED,letters:d.rainPeriods,allDayRain,label:'Słabe warunki',desc:'Mocne lub intensywne opady'};
  ...
  function ensureStyles(){
    if(document.getElementById('cfWeatherSignals11911Styles'))return;
    const s=document.createElement('style');s.id='cfWeatherSignals11911Styles';s.textContent=`
      ...
      @media (max-width:620px) and (orientation:portrait){
        #cfWeatherSlot .cf-w-days{min-height:68px}
        #cfWeatherSlot .cf-w-day{gap:3px;padding-top:6px;padding-bottom:6px}
        #cfWeatherSlot .cf-w-topline{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px}
        #cfWeatherSlot .cf-w-topline>.cf-signal{margin-left:0;min-height:10px}
        #cfWeatherSlot .cf-w-date-small{margin-top:0}
      }
  ...
  async function fetchModel(model){
    ...
    try{
      const p=new URLSearchParams({latitude:String(CFG.lat),longitude:String(CFG.lon),timezone:CFG.tz,forecast_days:String(CFG.days),models:model,daily:'precipitation_sum,precipitation_probability_max,wind_gusts_10m_max,weather_code',hourly:'precipitation,precipitation_probability'});
  ...
  function modelPeriods(row,date){
    ...
    for(let i=0;i<times.length;i++){
      ...
      const h=Number(t.slice(11,13)),key=periodForHour(h);
      out[key].sum+=num(prec[i]);out[key].maxPop=Math.max(out[key].maxPop,num(pop[i]));
    }
    return out;
  }
  ...
  function combine(rows){
    ...
    return dates.map((date,i)=>{
      ...
      const rainPeriods=['R','P','W'].filter(key=>{
        const sums=perModel.map(x=>x[key].sum),pops=perModel.map(x=>x[key].maxPop);
        const wetShare=sums.length?sums.filter(v=>v>=0.1).length/sums.length:0;
        return med(sums)>=0.2 || (avg(pops)>=40 && wetShare>=0.5);
      });
  ...
  function signalSignature(c){return `${c.dot}|${c.allDayRain?'ALL':c.letters.join('')}|${c.label}`;}
  ...
  function buildSignal(c){
    ...
    const baseDot=document.createElement('span');baseDot.className='cf-signal-dot';baseDot.style.background=c.dot;wrap.appendChild(baseDot);
    if(c.allDayRain){
      const rainDot=document.createElement('span');rainDot.className='cf-signal-dot';rainDot.style.background=BLUE;rainDot.setAttribute('aria-label','Opady przez cały dzień');wrap.appendChild(rainDot);
    }else{
  ...
  function apply(){
    if(!days.length)return;ensureStyles();
    ...
    const today=days[0],tc=today?classify(today):null,tileDesc=document.querySelector('#cfCompanyWeatherCard .cf-weather-tile-desc');if(tileDesc&&tc&&tileDesc.textContent!==tc.label)tileDesc.textContent=tc.label;
    ...
    if(title&&d){const c=classify(d);replaceSignal(title,d);const text=[...title.childNodes].find(n=>n.nodeType===Node.TEXT_NODE);if(!text||text.textContent!==c.label){[...title.childNodes].filter(n=>n.nodeType===Node.TEXT_NODE).forEach(n=>n.remove());title.append(document.createTextNode(c.label));}if(summary){const ico=(summary.textContent||'').trim().split(/\s+/)[0]||'';const val=`${ico} ${c.desc}`.trim();if(summary.textContent!==val)summary.textContent=val;}}
