(() => {
  ...
  function icon(d){if(d.rain>=4)return'🌧️';if(d.rain>=0.8)return'🌦️';if(d.gust>=45)return'💨';return d.confidence>=70?'🌤️':'⛅';}
  ...
  function styles(){
    if(document.getElementById('cfWeatherStylesV3'))return;
    const s=document.createElement('style');s.id='cfWeatherStylesV3';s.textContent=`
      ...
      @media(max-width:850px){.cf-company-weather-card{grid-column:1/-1!important}.cf-weather-tile-temp{font-size:36px}}@media(max-width:520px){.cf-w-day{font-size:9px}.cf-w-date-small{font-size:7px}.cf-weather-tile-days{gap:4px}.cf-weather-tile-day{padding:7px 2px}}
  ...
  function syncStripPosition(){
    ...
    if(companyGrid){host.style.display='none';return;}
  ...
  function tileMarkup(){
    ...
    return `<div class="cf-weather-tile-head"><div><div class="cf-weather-tile-title">Pogoda</div><div class="cf-weather-tile-now"><div class="cf-weather-tile-temp">${Math.round(t.tmax)}°</div><div class="cf-weather-tile-desc">${esc(text(t))}</div></div></div><span class="cf-w-dot" style="background:${t.status.color}"></span></div><div class="cf-weather-tile-days">${state.days.slice(0,5).map((d,i)=>{const di=dayInfo(d.date);return`<button type="button" class="cf-weather-tile-day" data-cf-weather-tile-day="${i}"><strong>${di.short}</strong><div class="cf-weather-tile-icon">${icon(d)}</div><div class="cf-weather-tile-range">${Math.round(d.tmax)}° <span>${Math.round(d.tmin)}°</span></div></button>`;}).join('')}</div>`;
  }
  ...
  function ensureDashboardTile(){
    const grid=document.getElementById('cfCompanyGrid');if(!grid)return;
  ...
  function watchUi(){
    let scheduled=false;
    const refresh=()=>{if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;syncStripPosition();ensureDashboardTile();});};
  ...
  async function load(host){state.error=false;host.innerHTML='<div class="cf-w-msg">Ładowanie prognozy pogody…</div>';ensureDashboardTile();try{const settled=await Promise.allSettled(CFG.models.map(getModel));let models=settled.filter(x=>x.status==='fulfilled').map(x=>x.value),fb=false;if(models.length<2){models=await fallback();fb=true;}const days=combine(models);if(!days.length)throw new Error('Brak dni');state={days,labels:models.map(x=>x.model.label),fallback:fb,error:false};renderStrip(host);ensureDashboardTile();}catch(e){console.warn('CleanFleet weather',e);showError(host);}}
