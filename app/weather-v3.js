(() => {
  ...
  function icon(d){
    if(d.rain>=4)return'🌧️';
    if(d.rain>=0.8)return'🌦️';
    if(d.gust>=45)return'💨';
    return d.confidence>=70?'🌤️':'⛅';
  }
  ...
  function styles(){if(document.getElementById('cfWeatherStylesV3'))return;const s=document.createElement('style');s.id='cfWeatherStylesV3';s.textContent=`
    ...
    .cf-w-days{display:grid;grid-template-columns:repeat(7,1fr);min-height:58px}.cf-w-day{border:0;border-right:1px solid #edf0ee;background:#fff;padding:7px 2px 6px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;cursor:pointer;color:#222;font:800 10px/1 system-ui,-apple-system,sans-serif}.cf-w-day:last-child{border-right:0}.cf-w-day:active{background:#f6f8f6}.cf-w-dot{width:8px;height:8px;border-radius:50%;display:block}.cf-w-date-small{font-size:8px;font-weight:700;color:#8b938e;letter-spacing:.01em}.cf-w-msg{min-height:50px;display:flex;align-items:center;justify-content:center;gap:8px;padding:8px 12px;font-size:11px;color:#727b75}.cf-w-retry{border:0;background:none;color:#68751d;font-weight:800;cursor:pointer}
    ...
    .cf-company-weather-card{grid-column:span 2!important;min-height:150px!important;padding:16px!important;text-align:left!important;cursor:default!important;background:#fff!important;border:1px solid var(--line,#e5e7e5)!important;color:var(--ink,#171a18)!important;display:block!important}
    .cf-weather-tile-head{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:12px}.cf-weather-tile-title{font-size:13px;font-weight:900}.cf-weather-tile-now{display:flex;align-items:flex-end;gap:8px}.cf-weather-tile-temp{font-size:40px;line-height:.95;font-weight:900}.cf-weather-tile-desc{font-size:11px;color:var(--ink-soft,#727b75);line-height:1.3;max-width:200px;padding-bottom:3px}.cf-weather-tile-days{display:grid;grid-template-columns:repeat(5,1fr);gap:6px}.cf-weather-tile-day{border:0;background:#f7f8f7;border-radius:10px;padding:8px 4px;cursor:pointer;text-align:center;color:inherit}.cf-weather-tile-day strong{display:block;font-size:10px}.cf-weather-tile-icon{font-size:20px;line-height:1.2;margin:2px 0}.cf-weather-tile-range{font-size:10px;font-weight:800}.cf-weather-tile-range span{color:#929993;font-weight:700}.cf-weather-tile-error{display:flex;align-items:center;justify-content:center;min-height:110px;font-size:12px;color:var(--ink-soft,#727b75)}
    @media(max-width:850px){.cf-company-weather-card{grid-column:1/-1!important}.cf-weather-tile-temp{font-size:36px}}
    @media(max-width:520px){#cfWeatherSlot{border-radius:10px}.cf-w-day{font-size:9px;padding:6px 1px}.cf-w-dot{width:7px;height:7px}.cf-w-date-small{font-size:7px}.cf-weather-tile-days{gap:4px}.cf-weather-tile-day{padding:7px 2px}.cf-weather-tile-desc{max-width:150px}}
  `;document.head.appendChild(s);}

  function placeUnderHeader(host){const headerTop=document.querySelector('.header-top');if(headerTop&&headerTop.parentElement){headerTop.insertAdjacentElement('afterend',host);return true;}return false;}
  ...
  function weatherTileHtml(){
    ...
    return `<div class="cf-weather-tile-head"><div><div class="cf-weather-tile-title">Pogoda · Studzienice</div><div class="cf-weather-tile-now"><div class="cf-weather-tile-temp">${Math.round(today.tmax)}°</div><div class="cf-weather-tile-desc">${esc(text(today))}</div></div></div><div class="cf-w-dot" style="background:${today.status.color};margin-top:3px"></div></div><div class="cf-weather-tile-days">${state.days.slice(0,5).map((d,i)=>{const di=dayInfo(d.date);return`<button type="button" class="cf-weather-tile-day" data-cf-weather-tile-day="${i}"><strong>${di.short}</strong><div class="cf-weather-tile-icon">${icon(d)}</div><div class="cf-weather-tile-range">${Math.round(d.tmax)}° <span>${Math.round(d.tmin)}°</span></div></button>`;}).join('')}</div>`;
  }
  function ensureDashboardTile(){
    ...
    tile.innerHTML=weatherTileHtml();
    tile.querySelectorAll('[data-cf-weather-tile-day]').forEach(b=>b.addEventListener('click',()=>{const d=state.days[Number(b.dataset.cfWeatherTileDay)];if(d)modal(d,state.labels,state.fallback);}));
  }
  function watchDashboard(){const obs=new MutationObserver(()=>ensureDashboardTile());obs.observe(document.documentElement,{childList:true,subtree:true});ensureDashboardTile();}

  function error(host){state.error=true;host.innerHTML='<div class="cf-w-msg">Nie udało się załadować prognozy pogody.<button type="button" class="cf-w-retry">Ponów</button></div>';host.querySelector('.cf-w-retry')?.addEventListener('click',()=>load(host));ensureDashboardTile();}
  async function load(host){state.error=false;host.innerHTML='<div class="cf-w-msg">Ładowanie prognozy pogody…</div>';ensureDashboardTile();try{const settled=await Promise.allSettled(CFG.models.map(getModel));let models=settled.filter(x=>x.status==='fulfilled').map(x=>x.value),fb=false;if(models.length<2){models=await fallback();fb=true;}const days=combine(models);if(!days.length)throw new Error('Brak dni');state={days,labels:models.map(x=>x.model.label),fallback:fb,error:false};renderStrip(host,state.days,state.labels,state.fallback);ensureDashboardTile();}catch(e){console.warn('CleanFleet weather',e);error(host);}}
  function mount(){const host=document.getElementById('cfWeatherSlot');if(!host)return;styles();placeUnderHeader(host);watchDashboard();load(host);}
