(() => {
  ...
  function styles(){
    if(document.getElementById('cfWeatherStylesV5'))return;
    const s=document.createElement('style');s.id='cfWeatherStylesV5';s.textContent=`
  ...
  function overviewModal(){
    ...
    el.addEventListener('click',e=>{if(e.target===el)close();});
    el.querySelector('.cf-w-close')?.addEventListener('click',close);
    el.querySelectorAll('[data-overview-day]').forEach(b=>b.addEventListener('click',()=>dayModal(state.days[Number(b.dataset.overviewDay)],true)));
    document.body.appendChild(el);
  }
  ...
  function renderStrip(host){
    ...
    host.querySelectorAll('[data-i]').forEach(b=>b.addEventListener('click',()=>dayModal(state.days[Number(b.dataset.i)],false)));
  }
  ...
  function ensureDashboardTile(){
    ...
    tile.querySelectorAll('[data-cf-weather-tile-day]').forEach(b=>b.addEventListener('click',()=>dayModal(state.days[Number(b.dataset.cfWeatherTileDay)],false)));
  }
