(() => {
  ...
  style.textContent = `
    ...
    @media(min-width:801px) and (orientation:landscape) and (max-height:760px){
      ...
      .contact .content{width:min(480px,42vw)}
    ...
    @media(min-width:801px) and (orientation:landscape) and (max-height:650px){
      ...
      .contact .content{width:min(450px,40vw)}
  ...
  function anchorBottomLayouts(){
    if (innerWidth <= 800 || innerWidth <= innerHeight) return;
    const nav = document.querySelector('.nav');
    const navH = nav?.getBoundingClientRect().height || 54;
    const baseBottom = Math.round(navH + Math.max(8, innerHeight * 0.012));

    const steps = document.querySelector('.system .steps');
    const head = document.querySelector('.system .system-head');
    if (steps && head) {
      steps.style.setProperty('top','auto','important');
      steps.style.setProperty('bottom',`${baseBottom}px`,'important');
      const stepsH = steps.getBoundingClientRect().height;
      head.style.setProperty('top','auto','important');
      head.style.setProperty('bottom',`${Math.round(baseBottom + stepsH + Math.max(14, innerHeight * 0.02))}px`,'important');
    }

    const contact = document.querySelector('.contact .content');
    if (contact) {
      contact.style.setProperty('top','auto','important');
      contact.style.setProperty('bottom',`${baseBottom}px`,'important');
      contact.style.setProperty('transform','none','important');
    }
  }
  ...
  function apply() {
    ...
    requestAnimationFrame(anchorBottomLayouts);
  }
